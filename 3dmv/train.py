...
def train(epoch, iter, log_file, train_file):
    train_loss = []
    model.train()
    start = time.time()
    ...
    for t,v in enumerate(indices):
        # v is the index of a mini-batch
        ...
        y = output.data
        # print(y)
        y = y.view(y.nelement() // y.size(2), num_classes)[:, :-1] # shape (batch * height, classes), take out last class
